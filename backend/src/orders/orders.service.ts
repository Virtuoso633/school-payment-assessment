...
//         school_id: sanitizedSchoolId, // Use sanitized version
...
//     // Log all environment variables related to payment (except sensitive ones)
...
//           })
//           .pipe(
//             // In the catch block for the HTTP request in initiatePayment
...
export class OrdersService {
  ...
  async findAllTransactions(paginationQuery: PaginationQueryDto): Promise<{ data: any[], total: number }> {
    const {
      ...
      school_id,
      search,
    } = paginationQuery as any;

    const skip = (page - 1) * limit;
    const sortOrder = order.toLowerCase() === 'asc' ? 1 : -1;
    const sortStage: PipelineStage.Sort = { $sort: { [sort]: sortOrder, '_id': sortOrder } };

    // Build match object for filters
    const match: any = {};
    ...
    if (start_date || end_date) {
      ...
      if (start_date) match['statusInfo.payment_time'].$gte = new Date(start_date);
      if (end_date) match['statusInfo.payment_time'].$lte = new Date(end_date);
    }

    // School filter (multi-select)
    if (school_id) {
      const schools = school_id.split(',').map((s: string) => s.trim());
  ...
  async findTransactionsBySchool(schoolId: string, paginationQuery: PaginationQueryDto): Promise<{ data: any[], total: number }> {
    ...
    // Build match object for filters
    const match: any = { school_id: schoolId };
